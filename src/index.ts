  ...
  return match(MedicineStorage.get(id), {
      Some: (Medicine) => {
          if (Medicine.creator.toString() !== ic.caller().toString()) {
              return Result.Err<typeof Medicine, string>('You are not authorized to access Medicine');
          }
          return Result.Ok<typeof Medicine, string>(Medicine);
      },
      None: () => Result.Err<typeof Medicine, string>(`Medicine with id:${id} not found`),
  });
}

query
export function getMedicineByTags(tag: string): Result<Vec<typeof Medicine>, string> {
...
// Search Medicine
query
export function searchMedicines(searchInput: string): Result<Vec<typeof Medicine>, string> {
...
update
export function completedMedicine(id: string): Result<typeof Medicine, string> {
    return match(MedicineStorage.get(id), {
        Some: (Medicine) => {
            if (!Medicine.assigned_to) {
                return Result.Err<typeof Medicine, string>('No one was assigned the Medicine');
            }
            const completeMedicine:typeof Medicine = { ...Medicine, status: 'Completed' };
            MedicineStorage.insert(Medicine.id, completeMedicine);
            return Result.Ok<typeof Medicine, string>(completeMedicine);
        },
        None: () => Result.Err<typeof Medicine, string>(`Medicine with id:${id} not found`),
    });
}
...
update
export function addMedicine(payload: MedicinePayload):  Result<typeof Medicine, string>{
    // Validate input data
...
update
export function addTags(id: string, tags: Vec<string>):  Result<typeof Medicine, string>{
    // Validate input data
    ...
    return match(MedicineStorage.get(id), {
        Some: (Medicine) => {
            if (Medicine.creator.toString() !== ic.caller().toString()) {
                return Result.Err<tyepof Medicine, string>('You are not authorized to access Medicine');
            }
            const updatedMedicine:typeof Medicine = { ...Medicine, tags: [...Medicine.tags, ...tags], updated_at: Opt.Some(ic.time()) };
            MedicineStorage.insert(Medicine.id, updatedMedicine);
            return Result.Ok<typeof Medicine, string>(updatedMedicine);
        },
        None: () => Result.Err<typeof Medicine, string>(`Medicine with id:${id} not found`),
    });
}
...
update
export function updateMedicine(id: string, payload: MedicinePayload):  Result<typeof Medicine, string>{
    return match(MedicineStorage.get(id), {
        Some: (Medicine) => {
            // Authorization Check
            if (Medicine.creator.toString() !== ic.caller().toString()) {
            ...
            const updatedMedicine:typeof Medicine = { ...Medicine, ...payload, updated_at: Opt.Some(ic.time()) };
            MedicineStorage.insert(Medicine.id, updatedMedicine);
            return Result.Ok<typeof Medicine, string>(updatedMedicine);
        },
        None: () => Result.Err<typeof Medicine, string>(`Medicine with id:${id} not found`),
...
update
export function deleteMedicine(id: string):  Result<typeof Medicine, string>{
    return match(MedicineStorage.get(id), {
        Some: (Medicine) => {
            // Authorization Check
            if (Medicine.creator.toString() !== ic.caller().toString()) {
        ...
        None: () => Result.Err<typeof Medicine, string>(`Medicine with id:${id} not found, could not be deleted`),
    });
}

// Assign a Medicine to a User
update
export function assignMedicine(id: string, assignedTo: string):  Result<typeof Medicine, string>{
    return match(MedicineStorage.get(id), {
        Some: (Medicine) => {
            if (Medicine.creator.toString() !== ic.caller().toString()) {
                return Result.Err<typeof Medicine, string>('You are not authorized to assign a Medicine');
            }
            const updatedMedicine:typeof Medicine = { ...Medicine, assigned_to: assignedTo };
            MedicineStorage.insert(Medicine.id, updatedMedicine);
            return Result.Ok<typeof Medicine, string>(updatedMedicine);
        },
        None: () => Result.Err<typeof Medicine, string>(`Medicine with id:${id} not found`),
...
update
export function changeMedicineStatus(id: string, newStatus: string):  Result<typeof Medicine, string>{
    return match(MedicineStorage.get(id), {
        Some: (Medicine) => {
            if (Medicine.creator.toString() !== ic.caller().toString()) {
                return Result.Err<typeof Medicine, string>('You are not authorized to change the Medicine status');
            }
            const updatedMedicine:typeof Medicine = { ...Medicine, status: newStatus };
            MedicineStorage.insert(Medicine.id, updatedMedicine);
            return Result.Ok<typeof Medicine, string>(updatedMedicine);
        },
        None: () => Result.Err<typeof Medicine, string>(`Medicine with id:${id} not found`),
...
query
export function getMedicinesByStatus(status: string): Result<Vec<typeof Medicine>, string> {
...
update
export function setMedicinePriority(id: string, priority: string):  Result<typeof Medicine, string>{
    return match(MedicineStorage.get(id), {
        Some: (Medicine) => {
            if (Medicine.creator.toString() !== ic.caller().toString()) {
                return Result.Err<typeof Medicine, string>('You are not authorized to set Medicine priority');
            }
            const updatedMedicine: typeof Medicine = { ...Medicine, priority };
            MedicineStorage.insert(Medicine.id, updatedMedicine);
            return Result.Ok<typeof Medicine, string>(updatedMedicine);
        },
        None: () => Result.Err<typeof Medicine, string>(`Medicine with id:${id} not found`),
...
query
export function getMedicinesByCreator(creator: Principal): Result<Vec<typeof Medicine>, string> {
...
query
export function getOverdueMedicines(): Result<Vec<typeof Medicine>, string> {
    const now = new Date().toISOString();
...
// Medicine Comments
update
export function addMedicineComment(id: string, comment: string):  Result<typeof Medicine, string>{
    return match(MedicineStorage.get(id), {
        Some: (Medicine) => {
            const updatedComments = [...Medicine.comments, comment];
            const updatedMedicine: typeof Medicine = { ...Medicine, comments: updatedComments };
            MedicineStorage.insert(Medicine.id, updatedMedicine);
            return Result.Ok<typeof Medicine, string>(updatedMedicine);
        },
        None: () => Result.Err<typeof Medicine, string>(`Medicine with id:${id} not found`),
